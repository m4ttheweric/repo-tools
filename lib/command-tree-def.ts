...
export const TREE: Record<string, CommandNode> = {
    ...
    subcommands: {
      secrets: {
        ...
        subcommands: {
          ...
          "sync-browser": {
            description: "Overlay-named browser dotenv → Secret repo-browser-secrets (mc-sandboxes)",
            module: "./commands/cloud.ts",
            fn: "browserSecretsSyncCommand",
            context: "worktree",
            args: [],
          },
          "sync-agent": {
            description: "Overlay-named agent credential files → Secret agent-credentials (mc-sandboxes)",
            module: "./commands/cloud.ts",
            fn: "agentCredentialsSyncCommand",
            context: "worktree",
            args: [],
          },
          "sync-attended-key": {
  ...
  db: {
    ...
    subcommands: {
      push: {
        description: "Dump local assured → cluster assured_tpl → recreate live assured from it",
  ...
  sandbox: {
    ...
    subcommands: {
      create: {
        ...
        context: "worktree",
        args: [
          { name: "Ticket", flag: "--ticket", type: "text", placeholder: "CV-1234", hint: "Linear ticket — derives the branch name and brief via the provisioning path" },
          { name: "Branch", flag: "--branch", type: "text", placeholder: "branch-name", hint: "Existing branch to sandbox (default: current branch)" },
          { name: "Job dir", flag: "--job", type: "text", placeholder: "~/jobs/cv-1234", hint: "Directory whose job.md becomes the brief" },
          ...
          { name: "Attended", flag: "--attended", type: "boolean", default: false, hint: "Attended lane: in-pod herdr server + sshd; attach with rt sandbox attach" },
          { name: "TUI account", flag: "--tui-account", type: "text", placeholder: "acct-3", hint: "agent-credentials key prefix for the attended lane's TUI credentials" },
    ...
    subcommands: {
      request: {
        ...
        args: [
          ...
          { name: "View", flag: "--view", type: "text", placeholder: "cvi-case" },
