...
describe("readClusterCredentials", () => {
  test("decodes base64 username/password from the kubectl Secret JSON", async () => {
    const secretJson = JSON.stringify({
      data: {
        username: Buffer.from("assured").toString("base64"),
    ...
    expect(out).toEqual({ ok: true, creds: { username: "assured", password: "hunter2" } });
...
describe("localDumpSizeBytes", () => {
  test("parses the byte count from the local psql query", async () => {
    const { exec, calls } = fakeExec({
      "psql postgres://postgres:postgres@localhost:5432/assured": { stdout: "148992\n", stderr: "", exitCode: 0 },
    });

    const size = await localDumpSizeBytes(exec);

    expect(size).toBe(148992);
    expect(calls[0]!.argv).toEqual(["psql", "postgres://postgres:postgres@localhost:5432/assured", "-tAc", "SELECT pg_database_size('assured');"]);
  });

  test("returns null when the local query fails (no local Postgres, etc.)", async () => {
    const { exec } = fakeExec({
      "psql postgres://postgres:postgres@localhost:5432/assured": { stdout: "", stderr: "psql: error: connection refused", exitCode: 2 },
    });
...
describe("SQL builders", () => {
  test("terminateConnectionsSql kills every other backend on the named db", () => {
    expect(terminateConnectionsSql("assured_tpl")).toBe(
      "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'assured_tpl' AND pid <> pg_backend_pid();",
    );
  });

  test("dropDatabaseSql is idempotent (IF EXISTS)", () => {
    expect(dropDatabaseSql("assured_tpl")).toBe('DROP DATABASE IF EXISTS "assured_tpl";');
  });

  test("createDatabaseSql without a template", () => {
    expect(createDatabaseSql("assured_tpl")).toBe('CREATE DATABASE "assured_tpl";');
  });

  test("createDatabaseSql from a template", () => {
    expect(createDatabaseSql("assured", "assured_tpl")).toBe('CREATE DATABASE "assured" TEMPLATE "assured_tpl";');
  });
});
...
describe("pushDatabase", () => {
  test("declining the confirmation prompt spawns no port-forward and runs no cluster command", async () => {
    ...
    const { exec, calls } = fakeExec({
      "psql postgres://postgres:postgres@localhost:5432/assured": { stdout: "0\n", stderr: "", exitCode: 0 },
    });
    ...
    expect(calls.every(c => c.argv[0] === "psql" && c.argv[1] === "postgres://postgres:postgres@localhost:5432/assured")).toBe(true);
...
describe("pushDatabase confirm summary", () => {
  test("names the cluster, local source db, and dump size before anything destructive", async () => {
    ...
    expect(summaries).toEqual([
      { cluster: "mattcloud cluster (mc-system/svc/postgres)", sourceDb: "local assured", dumpSizeBytes: 148992 },
    ]);
  });
});
...
describe("local source URL", () => {
  test("pg_dump and the size query hit the default local TCP URL, never the bare socket", async () => {
    ...
    expect(argvs).toContain(
      "pg_dump --no-owner --no-privileges -f /tmp/rt-test-dump.sql postgres://postgres:postgres@localhost:5432/assured",
    );
    expect(argvs.some(a => a === "pg_dump --no-owner --no-privileges assured")).toBe(false);
    expect(argvs.some(a => a.startsWith("psql -d assured"))).toBe(false);
  });

  test("a sourceUrl override reaches both local legs", async () => {
    const url = "postgres://me:pw@localhost:6543/assured";
    const { exec, calls } = fakeExec({
      [`psql ${url}`]: { stdout: "0\n", stderr: "", exitCode: 0 },
      "kubectl -n mc-system get secret postgres-credentials": {
        stdout: JSON.stringify({ data: { username: "YQ==", password: "Yg==" } }),
    ...
    expect(argvs).toContain(`psql ${url} -tAc SELECT pg_database_size('assured');`);
  });
});
...
describe("phase rendering seam", () => {
  test("every phase runs through the injected runPhase, in order", async () => {
    ...
    expect(out.ok).toBe(true);
    expect(seen).toEqual([
      "pg_dump local assured",
      "recreate assured_tpl from the dump",
      "recreate live assured from assured_tpl",
    ]);
  });
});
...
describe("version-matched dumper", () => {
  test("dumpVia.docker runs pg_dump inside the source container, streaming to the host spool", async () => {
    ...
    expect(dumpCall).toContain("docker exec 'assured-db-1' pg_dump");
...
describe("pushDatabase happy path", () => {
  ...
  test("dumps local assured, restores into assured_tpl, then recreates live assured from it", async () => {
    ...
    const tplTerminate = argvs.indexOf(
      `psql -h 127.0.0.1 -p 15432 -U a -d postgres -v ON_ERROR_STOP=1 -c SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'assured_tpl' AND pid <> pg_backend_pid();`,
    );
    const tplDrop = argvs.indexOf(`psql -h 127.0.0.1 -p 15432 -U a -d postgres -v ON_ERROR_STOP=1 -c DROP DATABASE IF EXISTS "assured_tpl";`);
    const tplCreate = argvs.indexOf(`psql -h 127.0.0.1 -p 15432 -U a -d postgres -v ON_ERROR_STOP=1 -c CREATE DATABASE "assured_tpl";`);
    const tplRestore = argvs.indexOf(
      `psql -h 127.0.0.1 -p 15432 -U a -d assured_tpl -v ON_ERROR_STOP=1 -f /tmp/rt-test-dump.sql`,
    );
    ...
    const liveTerminate = argvs.indexOf(
      `psql -h 127.0.0.1 -p 15432 -U a -d postgres -v ON_ERROR_STOP=1 -c SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'assured' AND pid <> pg_backend_pid();`,
    );
    const liveDrop = argvs.indexOf(`psql -h 127.0.0.1 -p 15432 -U a -d postgres -v ON_ERROR_STOP=1 -c DROP DATABASE IF EXISTS "assured";`);
    const liveCreate = argvs.indexOf(`psql -h 127.0.0.1 -p 15432 -U a -d postgres -v ON_ERROR_STOP=1 -c CREATE DATABASE "assured" TEMPLATE "assured_tpl";`);
    ...
    if (out.ok) {
      expect(out.message).toContain("assured_tpl");
...
describe("pushDatabase failures mid-flight", () => {
  ...
  test("a failed template restore reports the failure without touching the live db", async () => {
    const { exec, calls } = fakeExec({
      ...
      "psql -h 127.0.0.1 -p 15432 -U a -d postgres -v ON_ERROR_STOP=1 -c DROP DATABASE IF EXISTS \"assured_tpl\";": { stdout: "", stderr: "", exitCode: 0 },
      "psql -h 127.0.0.1 -p 15432 -U a -d postgres -v ON_ERROR_STOP=1 -c CREATE DATABASE \"assured_tpl\";": { stdout: "", stderr: "", exitCode: 0 },
      "psql -h 127.0.0.1 -p 15432 -U a -d postgres -v ON_ERROR_STOP=1 -c SELECT pg_terminate_backend": { stdout: "", stderr: "", exitCode: 0 },
      "psql -h 127.0.0.1 -p 15432 -U a -d assured_tpl -v ON_ERROR_STOP=1": { stdout: "", stderr: "syntax error near FOO", exitCode: 3 },
    });
    ...
    expect(out).toEqual({
      ok: false,
      code: "cluster",
      message: expect.stringContaining("restore into assured_tpl failed: syntax error near FOO"),
    });
    expect(calls.some(c => c.argv.join(" ").includes(`DROP DATABASE IF EXISTS "assured";`))).toBe(false);
  });

  test("a failed live recreate names the recovery path (assured_tpl still holds the fresh dump)", async () => {
    const { exec } = fakeExec({
      ...baseScript(),
      "pg_dump": { stdout: "-- dump --", stderr: "", exitCode: 0 },
      "psql -h 127.0.0.1 -p 15432 -U a -d assured_tpl -v ON_ERROR_STOP=1": { stdout: "", stderr: "", exitCode: 0 },
      "psql -h 127.0.0.1 -p 15432 -U a -d postgres -v ON_ERROR_STOP=1 -c CREATE DATABASE \"assured\" TEMPLATE \"assured_tpl\";": {
        stdout: "",
    ...
    if (!out.ok) {
      ...
      expect(out.message).toContain("assured_tpl still holds the fresh dump");
