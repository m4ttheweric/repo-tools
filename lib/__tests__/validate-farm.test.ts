...
describe("resolveRepoId", () => {
  ...
  beforeEach(() => {
    ...
    const dir = join(overlayRoot, "assured-dev");
    mkdirSync(dir);
    writeFileSync(
      join(dir, "repo.jsonc"),
      `{\n  // the mirror origin\n  "origin": "https://gitlab.com/assured/assured-dev.git",\n  "defaultBranch": "master",\n}\n`,
    );
  ...
  test("matches the overlay dir by origin URL", () => {
    expect(resolveRepoId("https://gitlab.com/assured/assured-dev.git", overlayRoot)).toBe("assured-dev");
  });

  test("matches across ssh/https spellings of the same origin", () => {
    expect(resolveRepoId("git@gitlab.com:assured/assured-dev.git", overlayRoot)).toBe("assured-dev");
    expect(resolveRepoId("ssh://git@gitlab.com/assured/assured-dev", overlayRoot)).toBe("assured-dev");
  });

  test("matches the reverse direction: ssh-form overlay, https-form worktree", () => {
    const dir = join(overlayRoot, "ssh-overlay");
    mkdirSync(dir);
    writeFileSync(join(dir, "repo.jsonc"), `{ "origin": "git@gitlab.com:assured/other.git" }\n`);
    expect(resolveRepoId("https://gitlab.com/assured/other", overlayRoot)).toBe("ssh-overlay");
    expect(resolveRepoId("https://gitlab.com/assured/other.git", overlayRoot)).toBe("ssh-overlay");
  });
...
describe("normalizeOrigin", () => {
  test("all spellings of one repo normalize identically", () => {
    const want = "gitlab.com/assured/assured-dev";
    expect(normalizeOrigin("git@gitlab.com:assured/assured-dev.git")).toBe(want);
    expect(normalizeOrigin("https://gitlab.com/assured/assured-dev")).toBe(want);
    expect(normalizeOrigin("ssh://git@gitlab.com/assured/assured-dev.git")).toBe(want);
  });
});
...
function run(over: Partial<Run> = {}): Run {
  return {
    id: "r1", repoId: "assured-dev", tree: "t1", manifestHash: "m1",
...
describe("createControllerClient", () => {
  test("submit POSTs the request and returns runId + cached", async () => {
    ...
    const req = {
      repoId: "assured-dev", tree: "t1", manifestHash: "m1",
...
describe("resolveReceiverSshKey", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(root, "assured-dev"));
    writeFileSync(
      join(root, "assured-dev", "repo.jsonc"),
  ...
  test("MC_RECEIVER_SSH_KEY wins over the overlay key — explicit override", () => {
    expect(resolveReceiverSshKey("assured-dev", { MC_RECEIVER_SSH_KEY: "/env/key" }, root)).toBe("/env/key");
  });

  test("falls back to the overlay receiverSshKey with ~ expanded", () => {
    expect(resolveReceiverSshKey("assured-dev", { HOME: "/home/u" }, root)).toBe("/home/u/.ssh/mattcloud-mirror");
  });
  ...
  test("receiverSshEnv pins GIT_SSH_COMMAND from the overlay key", () => {
    expect(receiverSshEnv({ HOME: "/home/u" }, "assured-dev", root)).toEqual({
...
describe("receiverSshRemedy", () => {
  test("host key failure names the known_hosts entry and the rescan command", () => {
    const remedy = receiverSshRemedy("Host key verification failed.\nfatal: Could not read from remote repository.", {
      url: "ssh://git@localhost:2222/repos/assured-dev.git",
    });
    expect(remedy).toBe(
  ...
  test("publickey rejection points at the overlay receiverSshKey", () => {
    const remedy = receiverSshRemedy("git@localhost: Permission denied (publickey).", { repoId: "assured-dev" });
    expect(remedy).toBe(
      "receiver rejected the ssh key — check receiverSshKey in ~/.rt/repos/assured-dev/repo.jsonc (MC_RECEIVER_SSH_KEY overrides)",
    );
  });
...
describe("pushSnapshot", () => {
  ...
  // receiverSshKey for assured-dev, which would leak into the env asserts.
  ...
  test("pushes commit:refs/snapshots/<tree> to the receiver repo URL from the worktree", async () => {
    ...
    const out = await pushSnapshot({
      repoId: "assured-dev", commit: "c1", tree: "t1", cwd: "/wt", spawn, env: {}, reposRoot: emptyRoot,
    });
    expect(out.ok).toBe(true);
    expect(calls[0]!.argv).toEqual([
      "git", "push", "ssh://git@127.0.0.1:2222/repos/assured-dev.git", "+c1:refs/snapshots/t1",
    ]);
  ...
  test("threads MC_RECEIVER_SSH_KEY into the push env as GIT_SSH_COMMAND", async () => {
    const { spawn, calls } = fakeSpawn(0);
    await pushSnapshot({
      repoId: "assured-dev", commit: "c1", tree: "t1", cwd: "/wt", spawn,
