...
describe("loadRepoTracking", () => {
  ...
  test("valid map round-trips", () => {
    ...
    writeFileSync(path, JSON.stringify({ "assured-dev": "live", "claimview-tools": "poll" }));
    expect(loadRepoTracking(path)).toEqual({ "assured-dev": "live", "claimview-tools": "poll" });
  });
  ...
  test("array or scalar JSON means nothing tracked", () => {
    ...
    writeFileSync(arr, JSON.stringify(["assured-dev"]));
...
describe("trackingLevel", () => {
  test("unlisted repos default to off", () => {
    expect(trackingLevel({ "assured-dev": "live" }, "koguma")).toBe("off");
  });

  test("listed repos return their level", () => {
    expect(trackingLevel({ "assured-dev": "live", "x": "poll" }, "x")).toBe("poll");
  });
});
