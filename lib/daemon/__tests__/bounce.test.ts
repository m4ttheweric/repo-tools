...
const ALLOWED = new Set(["https://adjuster.localhost"]);
...
describe("resolveBounce", () => {
  test("302s to the return origin, preserving path + other query, dropping the return param", () => {
    const r = resolveBounce(
      "http://localhost:4001/callback?rt_return=https%3A%2F%2Fadjuster.localhost&code=abc&state=xyz",
      "rt_return", ALLOWED);
    expect(r.status).toBe(302);
    expect(r.location).toBe("https://adjuster.localhost/callback?code=abc&state=xyz");
  });
  ...
  test("preserves the request path other than /callback", () => {
    const r = resolveBounce(
      "http://localhost:4001/auth/cb?rt_return=https%3A%2F%2Fadjuster.localhost&code=1",
      "rt_return", ALLOWED);
    expect(r.location).toBe("https://adjuster.localhost/auth/cb?code=1");
  });
});
