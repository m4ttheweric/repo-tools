...
describe("deriveAppName", () => {
  test("uses package.json name, last segment, @ stripped, sanitized", () => {
    ...
    try {
      writeFileSync(join(d, "package.json"), JSON.stringify({ name: "@assured/Adjuster_App" }));
      expect(deriveAppName(d)).toBe("adjuster-app");
...
describe("portlessUrl", () => {
  test("no branch prefix -> bare name, no port", () => {
    expect(portlessUrl("adjuster", null)).toBe("https://adjuster.localhost");
  });
  test("linked worktree -> branch prefixed as subdomain, no port", () => {
    expect(portlessUrl("adjuster", "parking-lot-2")).toBe("https://parking-lot-2.adjuster.localhost");
  });
  test("scheme override", () => {
    expect(portlessUrl("adjuster", null, "http")).toBe("http://adjuster.localhost");
  });
});
...
describe("buildPortlessCommand opts", () => {
  ...
  test("with name + appPort -> flags before the inner command", () => {
    expect(buildPortlessCommand("vite", { appPort: 10001, name: "adjuster" }))
      .toBe("portless run --name adjuster --app-port 10001 vite");
  });
});
