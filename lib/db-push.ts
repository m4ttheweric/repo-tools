 ...
 * database. Dumps the local `assured` database, restores it into the
 * cluster's `assured_tpl` template, then recreates the live `assured`
...
export const LIVE_DB = "assured";
// The assured dev database runs in Docker publishing TCP 5432 with the
...
export const TEMPLATE_DB = "assured_tpl";
...
/** Local `assured` database size in bytes, or null when the local query fails. */
...
/**
 * Dump local `assured` → restore into cluster `assured_tpl` → recreate live
 * `assured` from the template, server-side. Every step past the confirm
...
export async function pushDatabase(opts: {
  ...
  try {
    ...
    const dump = await runPhase("pg_dump local assured", () => opts.exec(dumpArgv));
    if (dump.exitCode !== 0) {
