...
function makeMattstackDir(): string {
  ...
  const claimviewPluginDir = join(dir, "plugins", "claimview");
  writeFile(join(claimviewPluginDir, ".claude-plugin", "plugin.json"), JSON.stringify({ version: "0.3.0" }));
  writeFile(join(claimviewPluginDir, "attachments", "watch-ci-domain", "SKILL.md"), DOMAIN_SKILL_MD);
  writeFile(join(claimviewPluginDir, "attachments", "watch-ci-domain", "ci-config.json"), CI_CONFIG_JSON);
...
function computeGolden(mattstackDir: string) {
  const roots: PluginRoots = {
    byName: {
      ...
      claimview: { dir: join(mattstackDir, "plugins", "claimview"), version: "0.3.0" },
    },
  };
  ...
  const domain = loadAttachment("claimview:watch-ci-domain", "domain", roots);
