...
- Real-engine proof with the branch's compiler (the dev wrapper runs the shared `main` checkout, so it is not this branch): from this worktree's root, `bun run cli.ts skills compile --pack mattstack --pack-dir <mattstack checkout> --mattstack-dir <mattstack checkout>` is not the form either (that flag expects the test mirror); instead run `bun run cli.ts skills check --pack mattstack` and `bun run cli.ts skills check --pack claimview` with the working directory set to the mattstack-skills checkout and the team pack respectively (the check reads the installed plugin caches as usual, only the compiler code comes from this worktree), which is expected to report the internal verbs (item 1) and the four engines with unbound slot headings (item 3: review, self-review, receive-review, shepherdr) stale, the proof that the compiler output changed where intended; the team pack recompiles in its own release.
