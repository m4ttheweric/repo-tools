...
- `commands/runs.ts`: one exported function per subcommand doing argument parsing, `RT_RUN_DB` resolution, the call into `write.ts`, emission, and printing. Registered in `lib/command-tree-def.ts` under `runsSubcommands` beside `show` and `abandon`, with help text matching the surface above. The `runs` node's description drops "read-only".
