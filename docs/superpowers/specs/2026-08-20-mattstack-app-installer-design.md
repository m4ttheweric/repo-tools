...
| L5 suite artifacts | BOARD/GITQ/DECK/SKILLS/FB | board + gitq binaries (publish rt-client; config from team.jsonc + stores, no `.env`/config.json; skills as plugins; state.db), deck release + SMAppService-owned platform registration + private node/portless + privileged helper + our own proxy plist, fast-browser artifact + own setup + marketplace source fix, mattstack marketplace = MAT-360 meta repo, claimview pack requirements + `dev-ports.state.json` writer + local-db-mcp decision, rt-context extension via daemon | MAT-384 for TLD/managed apps; settings lane for key names (§13.2) |
