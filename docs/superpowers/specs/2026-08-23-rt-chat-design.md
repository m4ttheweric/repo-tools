...
4. `<repo>-<branch>`, slugified (`assured-cv-2299`)
5. `<user>-<host>`, slugified

On collision inside a room, a numeric suffix is appended at join
(`assured-cv-2299-2`). The **resolved** handle is written to `chat_members`
...
```
joined #buidl as assured-cv-2299 · 1 member · you are alone here
```
...
```
$ rt chat rooms
#cv-2299   3 members   2 unread (1 mention)   last 4m ago
#build     6 members   —                      last 2h ago

$ rt chat read
#build
  14:22  repo-tools-main   @assured-cv-2299 the events-bus migration landed, you're unblocked
```
