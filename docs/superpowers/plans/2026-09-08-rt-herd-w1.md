...
export interface HerdJobRow {
  ...
  handle: string; status: HerdJobStatus;
...
export interface HerdStore {
  ...
  upsertJob(input: { herd: string; name: string; worktree: string; branch?: string | null; pane?: string | null; agentSession?: string | null; agentId?: string | null; handle: string; status: HerdJobStatus }): HerdJobRow;
...
describe("herd-store", () => {
  ...
  test("upsertJob inserts then updates the same (herd, name) row", () => {
    const s = store();
    const h = herd(s);
    const j = s.upsertJob({ herd: h.id, name: "cv-1", worktree: "/w/cv-1", handle: "cv-1", status: "spawning" });
    expect(j).toMatchObject({ herd: h.id, name: "cv-1", status: "spawning", pane: null });
    const j2 = s.upsertJob({ herd: h.id, name: "cv-1", worktree: "/w/cv-1", handle: "cv-1", status: "spawning", pane: "w1:p1", agentSession: "sess-w" });
    expect(j2.pane).toBe("w1:p1");
...
interface JobColumns { herd: string; name: string; worktree: string; branch: string | null; pane: string | null; agentSession: string | null; agentId: string | null; handle: string; status: HerdJobStatus; lastGate: string | null; lastReport: number | null; createdAt: number; updatedAt: number }

const toHerd = (r: HerdColumns): HerdRow => ({ ...r, hidden: r.hidden === 1 });
const toJob = (r: JobColumns): HerdJobRow => ({ ...r });
...
export function createHerdStore(opts: { dbPath: string; log: Logger }): HerdStore {
  ...
  return {
    ...
    upsertJob(input) {
      ...
      if (!existing) {
        db.run(
          "INSERT INTO herd_jobs (herd, name, worktree, branch, pane, agentSession, agentId, handle, status, lastGate, lastReport, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL, ?, ?)",
          [input.herd, input.name, input.worktree, input.branch ?? null, input.pane ?? null, input.agentSession ?? null, input.agentId ?? null, input.handle, input.status, now, now],
        );
      } else {
        db.run(
          "UPDATE herd_jobs SET worktree = ?, branch = ?, pane = ?, agentSession = ?, agentId = ?, handle = ?, status = ?, updatedAt = ? WHERE herd = ? AND name = ?",
          [input.worktree, input.branch ?? existing.branch, input.pane ?? existing.pane, input.agentSession ?? existing.agentSession, input.agentId ?? existing.agentId, input.handle, input.status, now, input.herd, input.name],
        );
      }
...
**Interfaces:**
- Produces: `ClaudeInvocation.env?: Record<string, string>`; `agent:start` payload gains `env?: Record<string, string>` and `herdrSocket?: string`. With `herdrSocket`, the herdr launch uses `defaultHerdrRunner({ ...process.env, HERDR_SOCKET_PATH: herdrSocket })` for that call only.
...
Expected: FAIL on the four new tests (env ignored; unknown option).
...
In `packages/rt-client/src/commands.ts` line 543, extend the `agent:start` payload type with `env?: Record<string, string>; herdrSocket?: string`.

In `packages/rt-client/src/client.ts` `agentStart`, forward `env` and `herdrSocket` when defined (find the loop that copies optional keys and add both names).
...
export interface HerdJobInfo { herd: string; name: string; worktree: string; branch: string | null; pane: string | null; agentSession: string | null; agentId: string | null; handle: string; status: "spawning" | "active" | "at-gate" | "at-milestone" | "done" | "closed" | "crashed"; lastGate: string | null; lastReport: number | null; createdAt: number; updatedAt: number }
export interface HerdStatusData { herd: HerdInfo; jobs: Array<HerdJobInfo & { openGate: string | null; paneStatus: string | null }>; unread: number; lifecycleConnected: boolean; hiddenUp: boolean | null }
// Commands entries
...
describe("herd:start", () => {
  ...
  test("keeps an already-signed-in handle when sign-in returns one", async () => {
    const { h, store } = harness({
      chat: {
        "chat:sign-in": async () => ({ ok: true, data: { handle: "kai", baseHandle: "kai", sessionId: "sess-shep", room: null } }),
        "chat:join": async (p: any) => ({ ok: true, data: { handle: p.handle, memberCount: 1, unread: 0 } }),
        "chat:post": async () => ({ ok: true, data: { id: 1, recipients: [], others: 0 } }),
        "chat:archive": async () => ({ ok: true, data: { room: "x", archivedAt: 1 } }),
        "chat:rooms": async () => ({ ok: true, data: { rooms: [] } }),
      } as unknown as HerdDeps["chat"],
    });
    const res = await h["herd:start"](START);
...
describe("herd:resume / status / close", () => {
  ...
  test("status reports lifecycleConnected and hiddenUp null for a visible herd", async () => {
...
export function createHerdHandlers(deps: HerdDeps) {
  ...
  async function statusData(herdId: string): Promise<HerdStatusData | null> {
    ...
    if (!herd) return null;
    const [panes, gates, unread] = await Promise.all([paneStatuses(herd.herdrSocket), openHerdGates(herdId), unreadFor(herd.shepherdHandle, herd.room)]);
    ...
    return { herd, jobs, unread, lifecycleConnected: deps.lifecycle.connected(herd.herdrSocket), hiddenUp: herd.hidden ? await deps.hidden.up() : null };
  }
  ...
  return {
    "herd:start": async (raw: unknown): Promise<CommandResult<"herd:start">> => {
      ...
      const runner = deps.herdrRunnerFor(herdrSocket);
      const created = await runner(["workspace", "create", "--label", workspaceLabel(id), "--no-focus"]);
      if (created.exitCode !== 0) return { ok: false, error: `herdr workspace create failed: ${created.stdout.slice(0, 200)}` };
...
Note `hidden.ensure()` is awaited BEFORE the store row exists so a failed hidden start leaves no half-registered herd. Keep it that way.
...
- After `agentHandlers` is built and after the gate handlers object is built (assign `const gateHandlers = createGateHandlers(...)` to a const before spreading), build:
...
and spread `...herdHandlers` into the returned map. `findRun` is `lib/runs/store.ts`'s export; `worktreeHandlers` is whatever the router already names its `createWorktreeHandlers(...)` result (read the file; use that identifier). `RT_DIR` is imported wherever the router gets paths from, or pass `jobsRoot` in from daemon.ts if the router has no `RT_DIR`; do whichever matches the existing pattern.
...
```ts
"herd:spawn": { payload: { herd: string; job: string; brief?: string; dir?: string; model?: string; effort?: string; account?: string }; data: { herd: string; job: string; pane: string; worktree: string; branch: string | null; agentId: string; sessionId: string; handle: string } };
...
describe("herd:spawn", () => {
  ...
  test("provisions, starts the agent in the herd workspace with env and handle, signs the pane in, records the job", async () => {
    ...
    expect(agentCalls[0]).toMatchObject({
      ...
      workspace: `herd: ${herd}`, tab: "cv-1", label: "cv-1", caller: `herd:${herd}`,
    ...
    expect(store.getJob(herd, "cv-1")).toMatchObject({ worktree: "/w/cv-1", branch: "cv-1", pane: "w9:p1", agentSession: "sess-w1", agentId: "ag-1", handle: "cv-1", status: "spawning" });
    expect(res.data).toMatchObject({ pane: "w9:p1", worktree: "/w/cv-1", sessionId: "sess-w1" });
    expect(readFileSync(join(dir, "herds", herd, "cv-1", "job.md"), "utf8")).toContain("do the thing");
  });

  test("--dir skips provisioning and a respawn without --brief reuses the stored job.md", async () => {
    const { h, agentCalls, worktreeCalls, herd } = await started();
...
```ts
    "herd:spawn": async (raw: unknown): Promise<CommandResult<"herd:spawn">> => {
      ...
      let worktree = str(p?.dir); let branch: string | null = null;
      if (!worktree) {
        ...
        worktree = prov.data.path as string; branch = prov.data.branch as string;
      }

      store.upsertJob({ herd: herdId, name, worktree, branch, handle: name, status: "spawning" });
      const started = await deps.agent["agent:start"]({
        ...
        label: name, caller: `herd:${herdId}`, workspace: herd.workspace, tab: name,
      ...
      store.upsertJob({ herd: herdId, name, worktree, branch, handle: name, status: "spawning", pane: rec.paneId ?? null, agentSession: rec.sessionId, agentId: rec.id });
      ...
      if (handle !== name) store.upsertJob({ herd: herdId, name, worktree, branch, handle, status: "spawning" });

      return { ok: true, data: { herd: herdId, job: name, pane: rec.paneId ?? "", worktree, branch, agentId: rec.id, sessionId: rec.sessionId, handle } };
    },
...
Note the sign-in uses the minted `rec.sessionId` directly (no `viaPane`), so it never waits on herdr's session detection. The `chat:sign-in` handler falls through to `sessionId` when `viaPane` is absent; confirm by reading `handlers/chat.ts` `chat:sign-in` before relying on it.
...
export interface HerdLifecycle {
  start(): void;                         // subscribe default socket + every active hidden herd's socket; hook the bus
  stop(): void;
...
function fx() {
  ...
  const subs: Array<{ sockPath: string; onState?: (c: boolean) => void; stopped: boolean }> = [];
  const subscribe = ((o: any) => { const s = { sockPath: o.sockPath, onState: o.onState, stopped: false }; subs.push(s); o.onState?.(true); return { stop() { s.stopped = true; }, connected: () => !s.stopped }; }) as any;
  ...
  return { store, gateStore, gate, bus, posts, timers, subs, lc, herd };
}

describe("herd-lifecycle", () => {
  test("start subscribes the default socket and every active hidden herd socket; watch adds one; stop stops all", () => {
    const { store, lc, subs } = fx();
    ...
    lc.start();
    expect(subs.map((s) => s.sockPath).sort()).toEqual(["/default.sock", "/hidden.sock"]);
...
**Interfaces:**
- Produces: `GatesStore.deadPanePushes(): GateRow[]` (rows with `nudge` set, `delivery.outcome === "dead-pane"`, `released === false`, status `answered` or `closed`); `GatePush.retryDeadPanes(): Promise<{ retried: number; delivered: number; gaveUp: number }>`, max 20 attempts per gate held in daemon memory.
...
```ts
test("deadPanePushes lists answered/closed nudged rows whose last push was dead-pane and are unreleased", () => {
  const s = store();
  ...
  s.close(s.open({ subject: "herd:h/j4", kind: "question", questions: qs(), nudge: { session: "w4" } }).row.id, "abandoned");
  const d = s.list({ subjectPrefix: "herd:h/j4" }).gates[0]!.id;
  s.markDelivery(d, "dead-pane");
...
```ts
    async retryDeadPanes() {
      ...
      for (const row of store.deadPanePushes()) {
        ...
        const phrase = row.status === "answered" ? GATE_ANSWERED_PHRASE(row.id) : GATE_CLOSED_PHRASE(row.id, row.closedReason);
        await pushToPane(row, phrase);
...
**Files:**
- Create: `lib/daemon/herd-session.ts`
...
describe("hidden herd session", () => {
  ...
  test("ensure spawns `herdr server` with HERDR_SESSION and no HERDR_SOCKET_PATH, then waits for the socket", async () => {
    let calls = 0;
    ...
    await s.ensure();
    expect(spawned[0]!.argv.slice(-1)).toEqual(["server"]);
...
describe("hidden verbs", () => {
  test("attend resolves the hidden pane's terminal and opens an attached tab in the caller's workspace", async () => {
    ...
    expect(run[4]).toContain("terminal attach term-7");
...
export function createHiddenSession(opts: {
  ...
  const spawn = opts.spawn ?? ((argv, env, logPath) => {
    ...
    const child = Bun.spawn(argv, { env, stdout: fd, stderr: fd, stdin: "ignore" });
    child.unref();
  });
  ...
  return {
    ...
    async ensure() {
      ...
      spawn([resolveHerdrBin(), "server"], envWithoutSocket({ HERDR_SESSION: HIDDEN_SESSION }), join(logDir, "herd-session.log"));
...
```ts
    "herd:attend": async (raw: unknown): Promise<CommandResult<"herd:attend">> => {
      ...
      try { termId = JSON.parse(got.stdout)?.result?.pane?.terminal_id; } catch { /* handled below */ }
      ...
      const attach = `env -u HERDR_SOCKET_PATH HERDR_SESSION=herd herdr terminal attach ${termId}`;
...
Port the exact `pane get` field name for the terminal id from `attachments/orchestration/shepherdr/scripts/attend.sh` line 45 in mattstack-skills (it reads the JSON path there); use that path, not `terminal_id`, if it differs, and update the test fixture to match.
...
describe("herd:wrap-up", () => {
  async function withTwoJobs() {
    ...
    if (!s.ok) throw new Error(s.error);
    const herd = s.data.herd;
    hx.store.upsertJob({ herd, name: "cv-1", worktree: "/w/cv-1", branch: "cv-1", handle: "cv-1", status: "done", pane: "w9:p1" });
    hx.store.upsertJob({ herd, name: "cv-2", worktree: "/w/cv-2", branch: "cv-2", handle: "cv-2", status: "done", pane: "w9:p2" });
    mkdirSync(join(hx.dir, "herds", herd, "cv-1"), { recursive: true });
    return { ...hx, herd, room: s.data.room };
  }

  test("runs exactly the flagged actions: panes, workspace, dispose, job dirs, archive", async () => {
    ...
    if (!res.ok) throw new Error(res.error);
    expect(res.data).toEqual({ closed: ["cv-1", "cv-2"], workspaceClosed: true, disposed: ["cv-1"], refused: [], deletedJobDirs: true, archived: true });
    expect(herdrCalls).toContainEqual(["pane", "close", "w9:p1"]);
    expect(herdrCalls).toContainEqual(["pane", "close", "w9:p2"]);
    expect(herdrCalls.some((c) => c[0] === "workspace" && c[1] === "close")).toBe(true);
    expect(worktreeCalls).toContainEqual({ verb: "dispose", p: { repoName: "gh:m4ttstack/rt", tree: "cv-1" } });
  ...
  test("with no flags it only marks the herd wrapped and reports a dispose refusal verbatim", async () => {
    ...
    if (!s.ok) throw new Error(s.error);
    refusing.store.upsertJob({ herd: s.data.herd, name: "cv-1", worktree: "/w/cv-1", branch: "cv-1", handle: "cv-1", status: "done" });
    const res = await refusing.h["herd:wrap-up"]({ herd: s.data.herd, dispose: ["cv-1"] });
    if (!res.ok) throw new Error(res.error);
    expect(res.data).toMatchObject({ closed: [], workspaceClosed: false, disposed: [], refused: [{ tree: "cv-1", reason: "unmerged work" }], deletedJobDirs: false, archived: false });
...
```ts
    "herd:wrap-up": async (raw: unknown): Promise<CommandResult<"herd:wrap-up">> => {
      ...
      for (const name of Array.isArray(p?.dispose) ? p!.dispose! : []) {
        ...
        if (!job || !job.branch) { refused.push({ tree: name, reason: "no rt-provisioned tree for this job" }); continue; }
        const r = await deps.worktree["worktree:dispose"]({ repoName: herd.repo, tree: job.branch });
        if (!r.ok) { refused.push({ tree: job.branch, reason: r.error }); continue; }
...
rt herd spawn --herd <id> --job <name> [--brief <file>] [--dir <path>] [--model M] [--effort E] [--account A] [--json]
...
export function buildSpawnPayload(args: string[]): Commands["herd:spawn"]["payload"] {
  ...
  if (!herd || !job) throw new Error("usage: rt herd spawn --herd <id> --job <name> [--brief <file>] [--dir <path>] [--model M] [--effort E] [--account A]");
...
export async function status(args: string[]): Promise<void> {
  ...
  if (json) { emit(true, data, ""); return; }
  console.log(`${data.herd.id}  room ${data.herd.room}  unread ${data.unread}  lifecycle ${data.lifecycleConnected ? "connected" : "OFF"}${data.hiddenUp === null ? "" : `  hidden ${data.hiddenUp ? "up" : "DOWN"}`}`);
  for (const j of data.jobs) console.log(`  ${j.name.padEnd(24)} ${j.status.padEnd(13)} pane ${j.pane ?? "-"}  ${j.paneStatus ?? "-"}${j.openGate ? `  gate ${j.openGate}` : ""}`);
}
...
/** herdr fake that answers workspace/tab/pane verbs the way agent-herdr.ts drives them, plus session.snapshot. */
function herdrForHerd(): ReturnType<typeof fakeHerdr> {
  const handler: FakeHerdrHandler = (method, params) => {
    if (method === "workspace.list") return { type: "workspaces", workspaces: [] };
    if (method === "workspace.create") return { type: "workspace_created", root_pane: { pane_id: "w1:p1", tab_id: "w1:t1", workspace_id: "w1" } };
    if (method === "tab.rename" || method === "pane.run" || method === "pane.close" || method === "workspace.close") return { type: "ok" };
...
describe("rt herd end to end", () => {
  ...
  beforeEach(async () => {
    ...
    herdr = herdrForHerd();
    await startDaemonForHome(home, { HERDR_SOCKET_PATH: herdr.sock, HERDR_BIN: "/usr/bin/false" });
  });
  ...
  test("worker ask pushes to the shepherd; shepherd answer nudges the worker", async () => {
    ...
    const spawned = await finished(runRt(["herd", "spawn", "--herd", herd.herd, "--job", "cv-1", "--brief", brief, "--dir", repo, "--json"], home));
    ...
    const asked = await finished(runRt(["herd", "ask", "--questions", JSON.stringify([{ id: "q", label: "Which?", multi: false, options: ["a", "b"] }]), "--json"], home,
      { HERD_ID: herd.herd, HERD_JOB: "cv-1", CLAUDE_CODE_SESSION_ID: "sess-worker" }));
    ...
    expect(JSON.parse(status.stdout).jobs[0]).toMatchObject({ name: "cv-1", status: "active" });
  });
});
```
...
Expected: PASS. First failures usually mean the fake herdr is missing a verb `agent:start` calls; add it to `herdrForHerd` from the daemon log under `<home>/.rt/logs/`.
