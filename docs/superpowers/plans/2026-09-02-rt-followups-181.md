...
- Never run the built binary or any `rt` verb against the real `~/.mattstack` from a test. The test preload fakes `HOME`; the e2e fixture copies to a tmpdir. The real-engine proof in Task 8 reads other checkouts but writes nothing into them without Matt's go-ahead.
...
- Modify: `commands/skills.ts:36,54` (imports), `:574-618` (`compileVerb`), `:657-668` (`tryCompileVerb`), `:690-710` (`compileTargets`), `:725-726,733,758,783` (`skillsCompile`), `:851-852,866` (`skillsCheck`)
...
async function compileCapturingLogs(pack: string, ms: string, manifest: string, extra: string[] = []): Promise<string[]> {
  const logs: string[] = [];
  const logSpy = spyOn(console, "log").mockImplementation((...args: unknown[]) => {
    logs.push(args.map(String).join(" "));
  });
  try {
    await skillsCompile(["--pack-dir", pack, "--mattstack-dir", ms, "--manifest", manifest, ...extra]);
  } finally {
    logSpy.mockRestore();
  }
  return logs;
...
describe("internal roster verbs", () => {
  test("an internal verb's vendored include is addressed from its attachments-side host dir and lints clean", async () => {
    ...
    const logs = await compileCapturingLogs(pack, ms, manifest);
...
Run: `bun test lib/skills/__tests__/placeholders.test.ts lib/skills/__tests__/compile.test.ts -t "stage"`
Expected: two FAILs, each showing the thrown message `... {{stage.dir}} used outside a stage` against the expected `used in a public verb`.
...
describe("verb.path end to end", () => {
  ...
  test("a --verb compile still renders a path to a sibling it is not emitting", async () => {
    const { pack, ms, manifest } = buildWithRosterVerbs();
    await compileCapturingLogs(pack, ms, manifest, ["--verb", "checkout"]);
...
Expected: FAIL. The `verb.path` tests throw `work: unknown placeholder {{verb.path:checkout}} at line 1`; the e2e tests fail with `rt skills: verb "checkout": checkout: unknown placeholder {{verb.path:receive-review}} at line 1` printed through the clean-exit path; the three reworded message assertions fail on the old `a fill may carry {{include}} only` text. The `ctx()` literal also fails to type-check until Step 3 adds the fields (bun runs it anyway; `tsc` is the gate).
...
Expected: FAIL. Placeholder and compile cases throw `unknown placeholder {{pack.path:...}}`; the fill case throws `a fill may carry {{include}}, {{verb.path}} or {{pack.path}} only`; the e2e case fails through the clean-exit path with the same message prefixed `stage "stage-plan": acme:plan-policy:`.
...
Expected: FAIL on the first two cases (`Expected: "a\n\nb"`, `Received: "a\n\n## Reviewer\n\n\n\nb"`) and the fence case; the other three pass already.
...
In `commands/__tests__/runs-write.test.ts`, change the assertion at line 81 (the "subcommands without RT_RUN_DB" test):
...
Expected: FAIL. The line-81 test receives `{ ok: false, error: "RT_RUN_DB is not set" }`; every fallback case receives that same error with code 2.
...
cd "<the claimview dir that prints>"
bun run "$RT_WT/cli.ts" skills check --pack claimview
```
...
**Placeholder scan.** Every code step carries the code. The two `<paste>` markers in the PR body are filled from Task 8 Step 3's output, and `<the claimview dir that prints>` from `skills packs`; both are runtime values, not plan gaps.
